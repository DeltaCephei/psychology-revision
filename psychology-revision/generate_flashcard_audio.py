"""Generate flashcard narration audio using ElevenLabs TTS.

Each file reads the question then the answer in a natural teaching style.
Files are named fc_l{level}_{index}.mp3 to match the JS array indices.
"""
...
# Flashcard data — mirrors the arrays in script.js exactly.
# Order matters: the index here must match the JS array index.
...
def generate_flashcard_audio():
    """Generate MP3 files for all flashcards."""
    client = ElevenLabs(api_key=API_KEY)
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    total = len(LEVEL2) + len(LEVEL3)
    done = 0

    for level, cards in [("2", LEVEL2), ("3", LEVEL3)]:
        for i, (question, answer) in enumerate(cards):
            filename = f"fc_l{level}_{i:02d}.mp3"
    ...
    print(f"\nDone! {total} flashcard audio files generated.")
